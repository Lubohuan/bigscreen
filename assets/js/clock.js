(function($) {
    ...
    jQuery.fn.MyDigitClock = function(options) {
        ...
        function showClock(id)
        {
            ...
            templateStr = templateStr.replace("{year}", getDD(year));
            templateStr = templateStr.replace("{month}", getDD(month));
            templateStr = templateStr.replace("{date}", getDD(date));
    ...
    jQuery.fn.MyDigitClock.defaults = {
        ...
        fontWeight: 'bold',
        background: '#fff',
        timeFormat: '{year}<span>-</span>{month}<span>-</span>{date}<span>&nbsp;&nbsp;&nbsp;</span>{HH}<span id="ch1">:</span>{MM}<span id="ch2">:</span>{SS}',
